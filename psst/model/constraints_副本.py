...
def power_balance(m, b, t, has_storage=False, has_non_dispatchable_generators=False):
    ...
    #constraint = m.NetPowerInjectionAtBus[b, t] + m.LoadGenerateMismatch[b,t] == 0
           

    return constraint

def net_power_at_bus_rule(m, b, t):
    
    try:
        _constraint = sum((1 - m.GeneratorForcedOutage[g,t]) * m.GeneratorBusContributionFactor[g, b] * m.PowerGenerated[g, t] for g in m.GeneratorsAtBus[b])
    except:
        _constraint = 0

    _constraint = _constraint - m.Demand[b, t]
...
def constraint_net_power(model):

    model.CalculateNetPowerAtBus = Constraint(model.Buses, model.TimePeriods, rule=net_power_at_bus_rule)
