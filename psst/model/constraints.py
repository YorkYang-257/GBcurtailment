...
def fix_first_angle_rule(m, t, slack_bus=1):
    return m.Angle[list(m.Buses)[slack_bus-1], t] == 0.0
...
def net_power_at_bus_rule(m, b, t, PriceSenLoadFlag=False):
    ...
    gen = sum( m.PowerGenerated[g, t] for g in m.GeneratorsAtBus[b])
    ...
    return m.NetPowerInjectionAtBus[b, t] == gen-demand
...
def enforce_reserve_down_requirements_rule(m, t):
    ...
    _constraint = _constraint <= m.TotalDemand[t]
...
def MCG_rule(m, t):
    _constraint = sum(m.MaximumPowerOutput[g] * m.UnitOn[g, t]for g in m.Generators)

    _constraint = _constraint >= 23000
    
    return _constraint

def Technical_constraints(model):
    model.tc1=Constraint(model.TimePeriods, rule=enforce_reserve_up_requirements_rule)
    model.tc2=Constraint(model.TimePeriods, rule=MCG_rule)
...
def enforce_zonal_reserve_down_requirement_rule(m, rz, t, PriceSenLoadFlag=False):
    ...
    if PriceSenLoadFlag is False:
        _constraint = _constraint <= (1 - m.ZonalDownReservePercent[rz]) * sum(
            m.Demand[d, t] for d in m.DemandInReserveZone[rz])
    else:
        _constraint = _constraint <= (1 - m.ZonalDownReservePercent[rz]) * \
                      (sum(m.Demand[d, t] for d in m.DemandInReserveZone[rz]) +
...
def enforce_ramping_rule_2(m, g, t):
    if t >= value(m.NumTimePeriods):
        ...
        return m.MaximumPowerAvailable[g, t] <= \
               ...
               m.ShutdownRampLimit[g] * (m.UnitOn[g, t] - m.UnitOn[g, t ])
...
def enforce_up_time_constraints_subsequent(m, g, t):
    if t <= value(m.InitialTimePeriodsOnLine[g]):
        ...
        if t == 0:
            return sum(
                m.UnitOn[g, n] for n in m.TimePeriods if t <= n <= (t + value(m.MinimumUpTime[g]) - 1)) >= \
        ...
        if t == 0:
            ...
            if m.UnitOn[g, t] - m.UnitOn[g, t - 1]==1:
                a=value(m.MinimumUpTime[g]-m.NumTimePeriods)+t
                m.ito[g].fix(a)
    ...
    if t <= value(m.InitialTimePeriodsOffLine[g]):
        ...
        if t == 0:
            return sum((1 - m.UnitOn[g, n]) for n in m.TimePeriods if
                       t <= n <= (t + value(m.MinimumDownTime[g]) - 1)) >= \
        ...
        if t == 0:
            ...
            if m.UnitOn[g, t - 1]-m.UnitOn[g, t]==1:
                a=value(m.MinimumDownTime[g]-m.NumTimePeriods)+t
                m.itf[g].fix(a)
            return sum(
...
def StageCost_rule(m, st):
    ...
    # return m.StageCost[st] == m.LoadPositiveMismatchPenalty * m.TimePeriodLength * \
