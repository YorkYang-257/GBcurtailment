...
def build_model(case,
    ...
    ramp_up_ramp_down_limits(model, scaled_ramp_up_limits=generator_df['RAMP_10'].to_dict(),
                             scaled_ramp_down_limits=generator_df['RAMP_10'].to_dict())
    ...
    for i, g in generator_df.iterrows():
        if g['NCOST'] == 2:
            ...
            if g['PMIN'] == g['PMAX']:
                ...
                small_increment = 0
            points[i] = np.linspace(g['PMIN'], g['PMAX'] + small_increment, num=2)
            values[i] = g['COST_0'] + g['COST_1'] * points[i]
        if g['NCOST'] == 3:
            points[i] = np.linspace(g['PMIN'], g['PMAX'], num=segments)
